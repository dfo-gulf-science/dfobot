...
METADATA_CSV_PATH = "/home/stoyelq/my_hot_storage/dfobot_working/goodness/goodness.csv"
METADATA_COLUMNS = ['result' ]

class GoodnessImageFolderCustom(ImageFolderCustom):
    ...
    def get_metadata(self, path):
        ...
        result = metadata_row["result"]
        uuid = metadata_row["uuid"]
        return out_tensor, result, uuid



def get_goodness_dataloader(batch_size, max_size=None, config_dict=None):
    ...
    image_datasets = {x: GoodnessImageFolderCustom(os.path.join(data_dir, x), data_transforms[x]) for x in ['train', 'val']}
