...
from model.ref_lines.ref_line_dataloader import get_ref_line_model, get_ref_line_dataloaders

from model.solver import get_run_log_dir


class RefLineSolver(ClassifierSolver):
    ...
    def __init__(self, model, criterion, optimizer, config_dict, **kwargs):
        ...
        dataloaders, dataset_sizes = get_ref_line_dataloaders(self.batch_size, self.max_data, config_dict=config_dict)
...
def run_ref_line_solver(device, config_dict=None, load_checkpoint=None):
    ...
    class_model = get_ref_line_model(device)
    ...
    solver = RefLineSolver(class_model,
                    criterion=criterion,
